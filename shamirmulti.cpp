...
namespace {
	...
	void checkSameLengths(const std::vector< std::vector<uint8_t> > & share_list) {
		//size_t previous(share_list[0].size());
...
std::vector<uint8_t> reconstruct(const std::vector<std::vector<uint8_t>> & share_list) {
	...
	checkDistinctIndicesSameThresholds(share_list);   /// remove this check and check in the loop of this function
...
void f() {
	...
	auto share_collection = distribute(secretmessage, 4, 8);
	...
	try {
		for (const auto & it: reconstruct(sharesubcol) ) {
			std::cout << it;
		}
		std::cout << std::endl;
